    ...
    <section className="relative overflow-hidden bg-muted/30 py-24 md:py-36">
      ...
      <div className="container relative mx-auto px-6 md:px-8">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {cards.map(({ icon: Icon, key, title, text, href }) => {
            const localTitle = t(`home.explore.cards.${key}.title`) || title;
            const localText = t(`home.explore.cards.${key}.text`) || text;
            return (
              <Link
                key={href}
                to={href}
              ...
              >
                ...
                <div className="relative">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {key === "involved" && (
                      <MembershipBadge variant="gold" label={t("home.explore.joinBadge") || "Join"} className="text-[9px] px-2 py-0.5" />
                    )}
                  </div>
