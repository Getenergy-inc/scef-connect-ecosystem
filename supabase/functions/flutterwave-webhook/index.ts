 ...
 * 
 * Handles payment notifications from Flutterwave with:
 * - Hash verification
...
serve(async (req: Request) => {
  ...
  console.log(`Flutterwave webhook request from IP: ${clientIP}`);
  ...
  try {
    ...
    if (signature !== flutterwaveSecretHash) {
      console.error(`Invalid Flutterwave signature from IP: ${clientIP}`);
    ...
    if (event.event === "charge.completed" && event.data?.status === "successful") {
      ...
      // Idempotency check - prevent duplicate processing using tx_ref or id
      const transactionRef = data.tx_ref || data.id?.toString();
      if (transactionRef && !checkIdempotency(transactionRef)) {
        console.log(`Skipping duplicate transaction: ${transactionRef}`);
        return new Response(JSON.stringify({ status: "success", duplicate: true }), {
          status: 200,
          headers: { 
            "Content-Type": "application/json", 
